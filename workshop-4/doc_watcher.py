"""
Workshop 4: Documentation Watcher with Auto-Post.

Monitors business-docs folder for changes and automatically:
...
ARCHITECTURE:
=============
Uses `watchdog` library for EVENT-BASED file system monitoring instead of polling.
This leverages OS-level notifications:
- macOS: FSEvents
- Linux: inotify
...
DEBOUNCING:
===========
File changes are debounced to handle rapid successive events (e.g., editor autosave).
Changes are batched for 2 seconds before processing.
...
======================
By default, only changes exceeding 100 words or 20 lines trigger posts.
Minor edits (typo fixes, formatting) are skipped. Customize with:
...
class DocChangeHandler:
    ...
    ====================
    When you save a file, editors often trigger multiple events:
    1. File modified (content change)
    2. File modified (metadata update)
    3. Sometimes a delete + create (atomic save)

    We debounce by:
    1. Collecting all events for DEBOUNCE_DELAY seconds
    2. Then processing them as a batch
    3. Resetting the timer if new events arrive

    This prevents duplicate processing and ensures we capture
    the final state after rapid changes.
    """
    ...
    def __init__(
        ...
        # Note: We don't store a db connection here because SQLite connections
        # can only be used in the thread that created them. Instead, we create
        # a new connection in _process_changes() which runs in a Timer thread.
    ...
    def on_change(self, event_type: str, src_path: str):
        ...
        # Only process .md files in the business-docs directory
        if path.suffix != ".md":
            return
...
def watch_mode_events(
    ...
    Uses OS-level file system events for instant change detection.
    Much more efficient than polling - zero CPU when idle.
    """
    try:
...
def watch_mode_polling(
    ...
    Use this if watchdog is unavailable or for debugging.
    Less efficient than event-driven mode.
    """
    print("=" * 60)
